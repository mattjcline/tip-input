...
`apps-script/.clasp.json` (created by `clasp create`) is gitignored since it
contains your real Sheet ID and Apps Script project ID - it's local-only and
regenerated by the command above, not needed to run the app.
